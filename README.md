...
一句话进入系统后，服务端的对话规划层只会给出五种结果，前端不做二次解释：

| 响应类型 | 触发例子 | 行为 |
|---|---|---|
| 知识回答 | 公司的请假制度是什么 | 走检索增强问答，答案带引用与后置核查 |
| 实时查询 | 查看今天考勤 / 这个月支出怎么样 / 查看上月支出 / 上周考勤情况 | 直接调用已注册的低风险查询，返回结构化结果卡 |
| 表单预填 | 我要请假三天 / 报销打车 86 元 / 打印机坏了帮我找人处理 | 抽取字段并打开原有提交弹窗，用户确认后才写入 |
| 页面导航 | 查看工单 / 打开企业全景 | 只解析服务端白名单 route key，绝不接受任意 URL |
| 受控动作 | 创建组织部门：客户成功，编码:CS | 生成操作预览，确认后才执行并记审计 |

确认策略：低风险汇总、普通列表、白名单导航和表单预填直接执行；敏感查询需要确认查看；写操作必须先预览再确认；批量或跨范围操作需要两次确认。确认请求同时校验角色权限、参数哈希、对象版本、预览有效期和幂等键，AI 只能从服务端注册表中选择 intent、action 和 route key，不能执行任意 SQL、任意 HTTP 请求或任意 URL。

时间区间由服务端解析成显式参数后再查询，因此历史问题不会被当成"今天"回答：

- 日：今天 / 昨天 / 前天 / 8月15日 / 2026-07-20 / 上个月15号
- 月：本月 / 上月 / 上个月 / 上上个月 / 7月 / 七月 / 今年3月 / 去年12月 / 2026-07
- 区间：本周 / 上周 / 上上周 / 最近7天 / 最近30天 / 最近一个月 / 今年 / 去年

每次查询只绑定一种区间形态（一天、一个月或一个显式起止范围），区间上限 366 天；说不清区间的"查看考勤""查看费用"仍然按页面导航处理。
...
<table>
  <tr>
    ...
    <td width="50%" valign="top">
      <strong>企业全景</strong><br/><br/>
      <img src="docs/screenshots/runtime/admin-dashboard.png" alt="企业全景" />
...
企业全景展示费用状态分布和月度趋势，点击状态或月份后可继续进入明细页面处理。
...
- 口语化故障反馈（"打印机坏了帮我找人处理"）也能生成工单预览
- 实时查询：今日考勤、本月费用流水、项目、合同、部门与工单列表
- 历史区间查询：上月/上个月/去年12月/7月支出、上周/最近30天/今年费用、指定某天或某月的考勤
- 白名单页面导航："查看工单""打开企业全景"
- 结构化预览
- 用户确认后正式写入
- 与审批、待办、通知等业务状态继续联动
- 管理端"管理助手"覆盖组织、排班、资产、财务、协作、薪酬、安全与部门记忆等受控写操作，全部需要确认并留审计

### 管理端一级入口

- 管理助手 `/admin/assistant`：一句话完成查询、导航与受控操作
- 企业全景 `/admin/overview`：经营指标汇总与逐层下钻，可带当前筛选跳转到管理助手
- `/admin/dashboard` 自动重定向到 `/admin/overview`，其余管理页面全部保留为深链
